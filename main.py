...
def gen_shapes(k, l):
    s, result = set(), []
    for n in divisors(k):
        if not set([n]).issubset(s):
            result.append(Rect(k // n, n, l))
        s = s.union(set([n, k//n]))
    return result
...
def place_shape(conv, shape: Rect, how, x, y):
    for i in range(shape.w):
        for j in range(shape.h):
            if how == VERTICALLY:
                conv[y+i][x+j] = shape.l
            else:
                conv[y+j][x+i] = shape.l
...
def min_convbelt_len(convbelts):
    result = int(10e9)
    for c in convbelts:
        result = min(result, convbelt_len(c))
    return result
    ...
    while True:
        ...
        for shape, horizontally in zip(validsh.shapes, validsh.how):
            if horizontally:
                ...
                inc = shape.h
            print(
                f"Placing \"{shape.l}\" : {shape}, {'horizontally' if horizontally else 'vertically'} in {(x, y)}.")
    ...
    while shapes:
        for shape in shapes:
            ...
            if how in {HORIZONTALLY, BOTH}:
                print(
                    f"Placing \"{shape.l}\" : {shape}, horizontally in {(x, y)}.")
                place_shape(convbelt, shape, HORIZONTALLY, x, y)
                inc = shape.w-1

            elif how == VERTICALLY:
                print(
                    f"Placing \"{shape.l}\" : {shape}, vertically in {(x, y)}.")
                place_shape(convbelt, shape, how, x, y)
                inc = shape.h-1
...
if __name__ == '__main__':
    ...
    items = [Item(55, 55, "a"), Item(10, 10, "d"),
             Item(10, 10, "b"), Item(10, 10, "c")]
    solve(items)
